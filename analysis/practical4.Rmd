...
At some point in this tutorial, we will need the gene annotation for the house sparrow genome. We'll copy this into our directory like so: 

```shell
# move into rehh
cd rehh
# copy from the shared directory
cp /shared/mark/genome_scan/misc/house_sparrow_chr8.gff .
```

We'll explain what this file is later, for now we should be ready to run the analysis in R.
...
We'll also read in the data we created in the [haplotype scan](practical3.html). **As always, you may need to change the paths here**

```r
# read in the selection scan data
house_bac <- read_tsv("~/rehh/house_bac_xpEHH.tsv")
```
...
```r
# read in the gff
gff <-  read_tsv("~/rehh/house_sparrow_chr8.gff", col_names = FALSE)
